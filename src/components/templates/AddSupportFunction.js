...
import { getTemplates, addSupportFunction } from "../../store/templates";
...
export default function AddSupportFunction({ open, id }) {
  const dispatch = useDispatch();
  const { list } = useSelector(getTemplates);

  const { supportFunctionsMeasure } = list?.filter(
    (template) => template._id === id
  )[0];

  const handleSubmit = (values) => {
    dispatch(addSupportFunction({ currentId: id, ...values }));
    return open(false);
  };
  ...
  return (
    <Container>
      <Header>
        <h5 className="m-0">Add Support Function</h5>
      </Header>
