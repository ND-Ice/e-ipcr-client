...
export default function EditSupportSuccessIndicator({
  open,
  supportFunctions,
  id,
}) {
  ...
  const sf = supportFunctions?.filter((sf) => sf?.id === funcId)[0];

  const successIndicator = sf?.successIndicators?.filter(
    (succ) => succ?.id === succId
  )[0];

  const handleSubmit = (values) => {
    dispatch(
      editSupportSuccessIndicator({ currentId: id, funcId, succId, ...values })
    );
    return open(false);
  };

  const handleDelete = () => {
    dispatch(deleteSupportSuccessIndicator({ currentId: id, funcId, succId }));
    return open(false);
  };
  return (
    <Container>
